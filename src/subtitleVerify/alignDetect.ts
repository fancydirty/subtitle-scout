...
/**
 * ≥ 此分数视为参考源可信：偏移量可以拿去校正。
 *
 * **0.75，不是最初的 0.9**（2026-07-31 按生产实测下调）。这次修正同时纠正了我对
 * 这个分数**含义**的理解，所以理由值得写清楚。
 *
 * ## 分数度量的是"内容相似度"，不是"对齐质量"
 *
 * 初版 0.9 来自 PoC：拿**同一份字幕自己平移**做实验，那种情况 Jaccard 必然 1.000，
 * 于是 0.9 看起来是个宽松的门槛。但真实场景里参考源是**另一条字幕**（内嵌的外语轨、
 * 或同目录的简繁双份），两者的断句、翻译取舍、是否翻译背景对白都不同。
 *
 * 生产实测（Peacemaker S02E02，bul 内嵌轨 554 条 vs 我们的中文字幕 562 条）：
 *
 *   全片 Jaccard = 0.886，而 **每一段的 offsetMs 都是 0 或 -100ms**（分 5 段各自检测）
 *   缺口来源：参考轨独有 54 秒说话时段、我们独有 92 秒；共有 11417 格、不共有 1462 格
 *
 * 也就是说 0.886 这个"不够高"的分数对应的是**时间轴完全正确**的字幕。那 146 秒差异是
 * 两条字幕的内容差异（一方翻了另一方没翻的台词、断句不同），与时间轴无关。
 *
 * 更直接的证据——在这对真实字幕上人为注入偏移：
 *
 *   注入 0 / 200 / 500 / 1000 / 2000 / 5000 / 8300 / 15000 ms
 *   → 检出偏移**全部精确**，而分数**恒为 0.886**
 *
 * 偏移多少都不改变分数，因为算法先滑到最佳位置再算重叠。所以分数回答的是
 * "**这个参考源可信吗**"，而 `offsetMs` 回答"**偏了多少**"。用 0.9 卡分数等于要求
 * 两条不同语言的字幕内容高度一致——那是个与对齐无关的、多数真实字幕对达不到的要求。
 *
 * ## 0.75 怎么来的
 *
 * 同一对字幕上的对照实测：
 *
 *   跨语言字幕对（时间轴正确）  0.886   ← 必须判可信
 *   帧率不匹配 24/23.976        0.733   ← 必须判不可信（平移修不好）
 *   帧率不匹配 25/23.976        0.404   ← 同上
 *   完全无关的另一集字幕        0.279   ← 同上
 *
 * 0.75 落在 0.733 与 0.886 之间，两侧余量分别是 0.017 和 0.136。取 0.75 而不是更低的
 * 0.74：帧率那档是这道闸最该拦住的东西（它会诱使用户点一个修不好的校正按钮），
 * 宁可让少数低相似度的合法参考落进灰区（表现为"无法验证"，诚实的沉默），
 * 也不能放一个平移修不好的进来。
 *
 * 余量偏窄是真实的局限，写在这里而不是假装它不存在：不同片源的帧率组合可能产出
 * 0.74~0.78 的分数，那时会判错。真正的解法是**检测帧率不匹配的形态**（偏移随时间线性
 * 增长）而不是靠一个标量分数——spec §5 已把"拉伸校正"排除在第一期外，那条形态检测
 * 属于同一批未来工作。
 */
 ...
 *
 * 保持 0.7（未随 CONFIDENT 下调）：实测"完全无关的另一集字幕"是 0.279、
 * 帧率不匹配最差 0.404，都远低于 0.7；而 0.733 那档帧率不匹配恰好落在
 * 0.7~0.75 的灰区里——那正是灰区该装的东西（"像是有点关系但不敢用"）。
 */
