 ...
 * 一次巡检最多处理几条。
 *
 * 取 5 的理由：单条目最坏成本由 referenceSource 的 `EMBEDDED_TOTAL_BUDGET_MS`(60s) 定档
 * （软预算，最坏仍可能超出一个单轨时长），5 × 60s ≈ 5 分钟，正好与下面的墙钟预算同量级——
 * 两个上限刻意校准到互相咬合，任一先到都止损。
 *
 ...
 */
export const VERIFY_SWEEP_MAX_ITEMS = 5
 ...
 *
 * 取 5 分钟：巡检 tick 是 15s 一拍，一次扫描占 5 分钟意味着最多让 20 拍的其它分支
 * （reap/dispatch）延后——这是可接受的上限，而不是好事。校验是增益功能，绝不该成为
 * 主循环的主要成本；真需要更快铺满全库时，靠"每 6h 再来 5 条"累积，不靠单拍加量。
 */
export const VERIFY_SWEEP_BUDGET_MS = 5 * 60_000

/**
 * 巡检间隔（时间门）。
 *
 * 取 6h 的理由：这是**兜底铺量**通道，不是热路径。字幕刚装上时最该被检测——但那一刻
 * find-subtitle worker 已经把文件放好，下一次巡检自然会捡到它；等最多 6h 换来的是
 * 每天最多 4 次 × 5 条的 ffmpeg 成本上限（软路由上完全无感）。间隔取短没有收益：
 * 候选集合是单调收缩的（检过一次、哈希不变就不再是候选，见 needsRecheck），
 * 缩短间隔只会让"已经全检完"的稳态世界里空转查询变频繁。
 */
