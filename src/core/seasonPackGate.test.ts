...
describe('runSeasonPackGate', () => {
  ...
  it('drops out-of-range filelist_index and non-subtitle extensions', () => {
    const map = { pairs: [
      ...
      { filelist_index: 3, episode_code: 'S02E02', confidence: 0.9, reason: 'x' },
    ], unmapped_files: [], reasons: [] }
    const r = runSeasonPackGate({ map, filelist, seasonEpisodes: seasonEps, minConfidence: 0.86 })
    expect(r.commit).toEqual([])
  })
  ...
  it('drops non-integer filelist_index cleanly (no crash)', () => {
    const map = { pairs: [
      { filelist_index: 1.5, episode_code: 'S02E01', confidence: 0.9, reason: 'x' },
      { filelist_index: NaN, episode_code: 'S02E02', confidence: 0.9, reason: 'x' },
    ], unmapped_files: [], reasons: [] }
    const r = runSeasonPackGate({ map, filelist, seasonEpisodes: seasonEps, minConfidence: 0.86 })
    expect(r.commit).toEqual([])
