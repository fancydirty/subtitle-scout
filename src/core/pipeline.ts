...
import { matchesEpisodeCode, type SeasonEpisode } from './episode.js'
...
// matchesEpisodeCode 已迁到 core/episode.ts（rankCandidates.ts 的 compactCandidates() relevance-aware
// 预筛也需要复用它；留在这里会和 episode.ts 之间形成循环 import）。此处重新导出保持向后兼容
// （pipeline.test.ts 等既有调用方仍从 './pipeline.js' 取），本文件内部仍用同一个导入的绑定。
export { matchesEpisodeCode }
