...
describe('executor', () => {
  ...
  // 退役T3 (W0-1)：no_safe_match 分支曾在 attempt>=2 时调用 diagnoseSeason 诊断钩子、
  // 命中 absolute_flat 就 upsertWanted 建 realign job（executor.ts 的旧 :345-357）。
  // 该钩子已删除——realign 出生权整体移交 v3 orchestrator 的 check_series_layout
  // + dispatch_realign_task（见 orchestratorAgent.tools.ts）。这条测试锁住：即使是第
  // 2 次（含以上）内容失败，no_safe_match 分支也只走既有的退避/dormant 结论，绝不
  // 再产生任何 realign upsert 副作用。
  it('no_safe_match 第 2 次内容失败（attempt>=2）：正常走内容退避轨，不再产生 realign upsert 副作用', async () => {
    mkEpisode('e1', 's1', 1, 1)
    jobs.upsertWanted({ kind: 'series_season', seriesId: 's1', season: 1 }, now)
    let job = jobs.claimNext(now)!
    const runEpisode = vi.fn(async () => ({ decision: 'no_safe_match' as const, journalPath: '/j.json' }))
    await executeJob(job, mkDeps(runEpisode)) // 第 1 次失败，attempt→1
    ...
    // 分支。推进 now 越过 1 天退避窗口，模拟真实的"第二次失败"。
    now += 2 * 86_400_000
    job = jobs.forceClaim('s1', 1, now)!
    await executeJob(job, mkDeps(runEpisode)) // 第 2 次失败，attempt→2——旧 executor 曾在此触发诊断钩子

    expect(jobs.get(job.id)!.state).toBe('failed') // 内容退避轨照常生效（未提前 dormant）
    const realignJob = lib.db.prepare(`SELECT * FROM jobs WHERE kind='realign' AND series_id='s1'`).get()
    expect(realignJob).toBeUndefined() // 出生权已移交 orchestrator，executor 自身绝不再建 realign job
  })
