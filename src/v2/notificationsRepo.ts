...
export function listRecentFound(db: ScoutDb, now: number): FoundRow[] {
  try {
    const rows = db.prepare(
      `SELECT work_id AS workId, title, season, episode, via, found_at AS foundAt
         FROM notifications
        WHERE found_at > ?
        ORDER BY found_at DESC, id DESC`,
    ).all(now - NOTIFICATION_RETENTION_MS) as FoundRow[]
    return rows
  } catch {
    return []
  }
...
export function listRecentFoundGrouped(db: ScoutDb, now: number): FoundGroup[] {
  ...
  for (const r of rows) {
    ...
    if (!g) {
      ...
      g = { workId: r.workId, title: r.title, season: r.season, episodes: [], latestAt: r.foundAt, via: r.via }
      byKey.set(key, g)
    }
