 ...
 *       原文是「距 2026-08-12 起再过一个发布周期，`runVerifySweep` 仍没有被接进
 *       daemonV2 的任何 pass。雪藏满两轮 = 没人真的要它」。
 *
 *       **时限作废**，理由是不可判定：本仓自 v0.1.0（2026-07-09）后再无 release tag，
 *       "一个发布周期"没有可查定义 → 这条判据永远停在"技术上还没到期"，
 *       是个永不触发的自毁开关。比没有判据更糟，因为它让人误以为这里有个会自己了结的机制。
 *
 *       用户原话（2026-08-14，被问及"字幕时间轴自动巡检还要不要"）：
 *         「a，先不管。b. 也不管。这俩都是目前先 archive 的功能。
 *           不删但也不接，等现在有的功能测试没毛病了再说。」
 *
 *       现判据是**人的裁决，不是时间**：
 *         · 封存（archive）：不删、不接线。算法与 213 条算法用例照常留在仓里跑
 *           （它们与孤儿族零耦合，2026-08-14 取证实测确认）。
 *         · 解封条件：字幕获取主链路经 live test 确认无毛病之后，由用户重新裁决
 *           "接通"还是"删除"。**到那时再问，不要自作主张删。**
 *
 *       ⚠️ 下面这条命令仍是"是否仍未接线"的判据，接线一旦发生，orphan 守卫会红：
 *       `rg 'runVerifySweep\(' src/v2/daemonV2.ts` 无输出 = 仍未接线。
 *
 * 反过来，**恢复**它只需要：在 daemonV2 加一个 pass 调 runVerifySweep（并写
 * `last_verify_sweep_at` meta 键），再把 TriagePage 挂回 AppShell。环一旦有了入口，
 * 上面 6 条端点立刻全部有意义。
 *
 * ⚠️ 2026-08-13 补记：上一句里的「把 TriagePage 挂回 AppShell」曾与旧前端清点表直接打架——
 * 那张表判「triage 已被雪藏 → 删」。用户裁决：**都留**，那张表已更正为「雪藏保留」。
