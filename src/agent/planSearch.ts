...
export async function planSearch(
  ...
  const prompt = [
    ...
    ...(isEpisode ? [
      ...
      `   metadata providers often return the English name even for zh-CN), USE YOUR OWN KNOWLEDGE.`,
      `   Prefer the OFFICIAL translation; when connector words vary (和/与/、), ASSRT indexes them as`,
      `   SEPARATE partitions — e.g. "Love, Death & Robots" is officially 爱，死亡和机器人 while uploaders`,
      `   also use 爱，死亡与机器人: generate BOTH as separate queries. Full names only, not fan`,
      `   abbreviations (爱死机 finds nothing). Only fall back to the bare "<original/English title>" when you genuinely`,
