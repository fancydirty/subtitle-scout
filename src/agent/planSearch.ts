...
export async function planSearch(
  ...
  const prompt = [
    ...
    ...(isEpisode ? [
      ...
      `   when a Chinese title is known use bare "<Chinese title>" (NO "第N季", NO year); otherwise use bare`,
      `   "<original/English title>". The bare title is what actually surfaces season packs on ASSRT — adding`,
