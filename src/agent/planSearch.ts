...
export async function planSearch(
  ...
  const prompt = [
    ...
    ...(isEpisode ? [
      ...
      `1. SEASON-level, to surface a whole-season pack. When a Chinese title is known use "<Chinese title> 第${season ?? 'N'}季"`,
      `   (strongest on ASSRT). Otherwise use the BARE "<original/English title>" with NO "Season N" and NO year`,
      `   (the bare title is what actually surfaces packs for shows without a Chinese title — do NOT add "Season N Year").`,
      `2. A per-episode fallback for a single-episode sub: "<Chinese title> 第${season ?? 'N'}季 第<episode>集"`,
