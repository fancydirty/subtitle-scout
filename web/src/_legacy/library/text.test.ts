...
import { seasonCoverageSentence, formatResultCount } from './text.js'
...
// ⚠️ Task ⑪：`describe('formatDuration')` 的两条**搬到了 web/src/lib/duration.test.ts**
// （不是删掉）。函数本体随之提到 `lib/duration.ts`——本目录已移入 `_legacy/`，而
// `settings/text.ts` 在用它，留在这里会造出 live → _legacy 的编译期依赖。断言逐字搬运。
