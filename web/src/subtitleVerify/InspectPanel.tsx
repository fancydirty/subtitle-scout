...
//
// ═══════════════════════════════════════════════════════════════════════════
// 🟡 2026-08-12「无活 UI 端点」裁决：本目录 7 个文件**今天零 import**，经裁决保留
// ═══════════════════════════════════════════════════════════════════════════
// 实测（可复核）：`rg "from '.*subtitleVerify/" web/src` 在本目录之外**零命中**——
// 打开这个面板的入口（_legacy/library 的 VerifyChip → EpisodeRow → SeasonAccordion）
// 早在 2026-08-07 就把 verify/onInspect 两个 prop 摘掉了，比 Task ⑪ 的旧页面下架还早。
// 所以它不是"被 _legacy 拖下水"，是**它自己那一层先被雪藏**。
//
// 为什么不删：它是字幕校验那一族的 UI 半边。整族（6 个后端端点 + 算法 + 本目录）的
// 完整事实链、留存理由与**可证伪的删除判据**写在 `src/v2/subtitleVerifyRepo.ts` 头注释。
// 一句话版本：后端算法是 246 条用例覆盖的真资产，缺的只是巡检那一根接线；删 UI 留算法
// 或删算法留 UI 都是病 A 的形状，要删就整族一起删。
//
// 什么时候可以删：见 subtitleVerifyRepo.ts 的判据 (a)/(b)。删的时候连同
// `_legacy/library/VerifyChip.tsx`、i18n 里的 verify_* 键一起走。
// ═══════════════════════════════════════════════════════════════════════════
//
// 三块（spec §4.2）：
