...
docker run --rm \
  ...
  node:22-slim \
  sh -lc 'apt-get update -qq && apt-get install -y -qq python3 make g++ ffmpeg ca-certificates >/dev/null && npm rebuild better-sqlite3 && npm install --no-save esbuild && npx tsx src/cli/index.ts sandbox-library --profile "$0" --root /tmp/sandbox-scratch/lib --cache-dir /tmp/sandbox-scratch/cache' \
  "$PROFILE"
