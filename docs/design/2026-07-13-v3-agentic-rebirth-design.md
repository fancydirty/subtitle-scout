...
- **dashboard 大改(明确记账)**:当前 web 只是最小接线("全仓校验"按钮触发链路)。基于 v3 主子代理树+活文档+worker_task 状态机的 dashboard **要重新设计**——展示主代理编排视图、子代理任务树(pending/claimed/done)、每个媒体缺哪些集/整季、合集包命中情况等。**但前端怎么设计都是后话:背后工作流跑通、跑顺、跑到满意(真站验收+间谍过家家实弹通过)再说,现在做界面是本末倒置。**用户明示(2026-07-13)。
- **auto-research 迭代 skill**:去分数化闭环(真跑→诊断根因→沉淀规则进 skill→防复发),信号=真实媒体项成败非标量分。调研中(task ad62e8bf)。
