...
- [x] **Step 1: Write failing test** for layout helpers
...
- [x] **Step 3: Minimal implementation** `paths.ts` + `types.ts`
...
- [x] **Step 1: Failing test** — SRT with timing + `{\an8}` → clean jsonl without timing; bilingual pending rows; ids stable

- [x] **Step 2: Implement** using `parseSrtCues` + ASS strip from pipeline/qualityGate
...
- [x] **Step 1: Failing test** — canonical timing preserved; tgt text applied; missing tgt fails closed
...
- [x] **Step 2: Implement** with injected `probe` / `extract` / `fetchSourceSub`
...
- [x] **Step 1: Failing tests** for anchors: single-hop, no Brave, workspace docs, freeze glossary, update_row, no hand-written final SRT

- [x] **Step 2: Write skill content** (factory if needed)
...
- [x] **Step 1: Tests** — path sandbox escape denied; update_row cannot change src; finalize requires out/target
...
- [x] **Step 1: Scripted/mock model test** asserting tool order includes resolve_source → materialize → freeze_glossary → update_row → merge → install (or held)

- [x] **Step 2: Implement worker assembly like findSubtitleWorker (allocate under `.subtitle-translate`)

- [x] **Step 3: Full `npm run check` + focused vitest + commit**
...
- [x] **Step 1: Default translate-item uses agent runner when flag/env allows; document**

- [x] **Step 2: Structural gate + term gate on bilingual before install**

- [x] **Step 3: `npx vitest run --maxWorkers=1` subset + `npm run check` + commit**
...
- [x] Serial tests green; update `docs/design/2026-07-21-campaign-run-log.md` with P1 status
- [x] Commit docs
