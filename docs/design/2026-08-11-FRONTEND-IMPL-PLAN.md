...
后端六步  ⓪ workbench  ① notifications  ② EpisodeState 八态
          ③ media_roots 健康列  ④ current 快照  ⑤ health 端点  ⑥ backdrop_path
前端四步  ⑦ shell 改造 + SSE Context 四层
          ⑧ 媒体库页 #/media   ⑨ 活动页 #/activity   ⑩ 通知页 #/notifications
收尾      ⑪ _legacy + docker build     ⑫ 终局审计 + 跨 task 收口
```

**最终基线**：
```
后端  144 文件 / 3369 用例 / 失败恰好 7 条（接手前的既有债务，面孔未变）
前端  109 文件 / 1329 用例 / 0 失败        （起点 78/863）
类型  npm run check + cd web && npx tsc --noEmit  均退出码 0
构建  docker build 退出码 0（本机验证，产物已核对：新页面在、旧页面组件已 tree-shaken）
```

| Task | commit | 审计产出 |
|---|---|---|
| ⓪ workbench | `8997bcf` | 🔴 写入点可整体删光而全绿 → 补 4 道守卫 |
| ① notifications | `8997bcf` | 🔴 无消费者=**已成立**的第 8 次（Task ⑩ 解除）|
| ② EpisodeState | `a5cf3f1` `e7898ff` | 🔴×4：假数字 / 虚报变异 / 规格偏离 / **R-F2 违反** |
| ③ media_roots | `07a9669` `57d32aa` | 🔴 `prepare` 在 try 外 = 那层保护**保护不到自己**（真回归）|
| ④ current | `e39e399` `56da668` | 🔴 跨阶段脏值 = F-6 的同型复发 |
| ⑤ health | `b217dd2` `940c5a5` | 🔴×2：**接线断言被行尾注释喂饱** / engineEnabled 同源是假话 |
| ⑥ backdrop | `8bb0d02` `5ff3061` | 🟡 队头阻塞=**永久饿死**（v43 `_checked_at`）|
| — flake | `218cb7b` | **真因是绑 `::` 却拨 `127.0.0.1`**，前人归因错误 |
| ⑦ shell + SSE | `3dba285` `020fa49` | 🔴 跨重启**静默失聪** + 我引入的 `??` 空串洞 |
| ⑧ 媒体库页 | `ebb3334` | 🔴 隔离测试守的是**恒真命题** / R-F2 根本没实现 |
| ⑨ 活动页 | `075fcbd` `718cbc2` | 🔴 首连误判成重连 / 断线永久挂着过期读数 |
| ⑩ 通知页 | `62d7c9d` `e1694bd` | 🔴 R-F3 守卫**作用域被文件清单锁死** |
| ⑪ 收尾 | `5c49dc8` | 🔴 移动后目录禁令会**静默变绿** |
| ⑫ 终局收口 | `935eb0c` | 🔴 **病 A 发生在交接处**：`health.roots` 无读取方 |

---

## 五、方法论沉淀：对抗审计在这一轮抓到了什么

**14 轮审计（4 轮设计 + 10 轮实施），没有一轮空手。** 抓到的东西可归为四类：

### 1. 假绿守卫（最高频，5 次）——「测试看起来在守，实际守的是别的东西」

| 形态 | 实例 |
|---|---|
| 断言被**注释**喂饱 | ⑤ 的接线断言：一行行尾注释让 4 条全绿 |
| 断言守的是**恒真命题** | ⑧ 的隔离测试：比对两个常量数组，与"有没有 import"零相关 |
| 全称否定的**作用域被清单锁死** | ⑩ 的 R-F3 守卫：第 5 个文件不在手写清单里 |
| 变异是**语义空操作** | ② 的"优先级"：单值列上调换 if 顺序不改变任何输出 |
| 探针**是聋的** | ⑩ 的 localStorage spy：此 jsdom 下该对象根本不存在 |

**共同解药**：① 判据落在**运行时**而非源码文本 ② 域**自动跟着代码走**（import 闭包）而非手写枚举
③ 每条守卫配**阳性对照**（解析器坏掉时会红，而不是恒绿）。
> 「一份从没红过的守卫和一份恒绿的装饰品，在 CI 上长得一模一样。」

### 2. 病 A 的三种新形态

- **写入点可整体删光而全绿**（⓪）——能力有了、无人读、且回退不报警
- **域锁死**（⑩）——守卫存在但作用域是手写的
- **⭐ 发生在交接处**（⑫）——每个 task 各自都对，每个都指着下一个说"读取方是它"，链条在最后一跳断掉

### 3. 病 B 的传染

② 的「生产实测 1192 行全 NULL」**源头是我**（写在设计文档里的过期观察），
subagent 照抄进源码注释包装成"实测"，审计登录生产库查证：**一行 NULL 都没有**。
教训：**引用任何数字前必须自己跑一遍**。我自己也在 ③ 的 commit message 里犯过一次
（引用 subagent 报的变异数字未复核，审计实测 3 红实为 2、5 红实为 6）。

### 4. 前人的注释也会错

flake 的根因被前人归因为"undici 连接池缓存陈旧连接"，逐条验证**全部不成立**
（Node 19+ 的 `close()` 会回收；单进程 300 轮实测 0/300）。
真因是**绑 `::` 却拨 `127.0.0.1`，两个地址族端口空间不互斥**——修完从"随机变红"变成"确定性 EADDRINUSE"。

### 5. subagent 的诚实度是可以要求出来的

把「0 红要如实说」写进 prompt 后，多个 subagent **主动报告自己的测试是装饰品**并补测试堵洞：
③ 的变异 (k)、⑥ 的 (d)、⑩ 的 M3/M7、⑫ 的"审计这个变异在结构上不可能让前端变红"。
最好的一次是 ⑩：它做了 **14 发自攻**，**3 发真的打穿了自己的守卫**，全部补上。
