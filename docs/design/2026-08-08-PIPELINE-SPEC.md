...
**开跑前的硬门（4-2 子代理提出，编排侧采纳）**：两个存量回填 pass 都是 daemon boot 时
的一次性代码路径，不是可显式调度的前置步骤。若 e2e 跑在一个从未 boot 过 daemonV2 的库上，
回填不会发生，而 e2e 会在**退化状态**下量命中率——正是 C21 想避免的那件事。故须先确认：
- `SELECT COUNT(*) FROM works WHERE provider_ids IS NULL` = 0（或等于已知的 skipped 数）
  —— 否则抓源腿退化成文本 query（假阴性多），命中率不是真实水平
- `SELECT COUNT(*) FROM files WHERE embedded_langs IS NULL` = 0（或等于 probe 失败数）
  —— 否则 judge 规则 2 与 D9 的 translatable 判定都在缺证据的状态下跑

**另一条待验的新耦合（3-4 子代理提出）**：字幕流现在消费快照前逐文件 stat（R12），
而生产不注入 `fileExists`（走真 `existsSync`）。这条耦合在单测里靠注入替身覆盖，
**未经生产端到端验证**——115 是 FUSE 挂载，stat 语义与本地盘不同，须在第 6 步实测。
