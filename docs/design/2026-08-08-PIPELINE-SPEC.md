...
| D12 | 字幕存在性检测分两档（具体机制见 §5 补）：**新增/指纹变化文件全量检测**；未变化文件按 `sub_recheck_at` 到点轮转，**每轮只查到点的那批** | 15 标签×3 扩展=45 次 stat/文件，115 网盘挂载放大 46 倍 |
...
| D18 | `sub_recheck_at` 的 **NULL 语义写死**：迁移时对全库随机打散到未来 7 天内（`now + random()*7天`），**不留 NULL** | 照字面写谓词 `sub_recheck_at <= now` 则 NULL 行永不命中（静默失效）；补 `IS NULL OR` 则首轮全库雪崩（几万文件 ×45 次 stat） |
...
现成实现 `findExternalSidecar`（`sidecar.ts:72-79`）是 15 种语言标签 × 3 种扩展名。
R24 要求连**未变化**文件也检测 → 大库几万文件 × 45 次 stat，115 是 FUSE 网盘挂载。
...
- 补 `IS NULL OR sub_recheck_at <= now` → 首轮全库命中，几万文件 × 45 次 stat **雪崩**
...
补上 R6/R7/R8/R24 与 D1/D2/D7/D11/D12。这是全部后续步骤的地基。
...
**前置依赖（D7，必须先做）**：`settingsRepo.addRoot` 加嵌套检测并拒绝祖先/后代关系。
否则 `/media` 与 `/media/115` 并存时，115 断连会让 D1 的差集删光 115 全库（C29）。

**D11 归属**：root 移除时的 files 清理必须挂进 `settingsRepo.removeRoot` 内部
（已验证它只清 8 张旧表、一行 files 不碰），不是挂在扫描里。
...
R24 让扫描承担"每个视频当前有没有同名中文字幕"这项职责，但全量做代价是 45 次 stat/文件
...
- 若补 `IS NULL OR sub_recheck_at <= now`，首轮全库命中 → 几万文件 × 45 次 stat 雪崩
