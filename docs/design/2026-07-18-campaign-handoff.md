...
> **对接手的 fresh Claude**：你没有前面的对话上下文。先读本文档「Part 0 交接」把状态、铁律、卡点吃进去，再决定下一步动作。
> **2026-07-18 上午更新：P5 已由用户拍板并部署验收通过（见「P5 收官记录」）。当前无卡点;下一役=鉴权 A1-A4 时机重估（问用户）。**
...
| 重复源 P1-P4b | ✅ 收官 | `e150b73` | 代码 + 生产部署 + 验收全绿 |
| 重复源 P5 | ✅ **已部署 + 验收通过（2026-07-18 上午，用户拍板"现在推"）** | — | 见下「P5 收官记录」 |
| 鉴权 A1-A4 | ⏳ 未开始 | — | 用户判断过"还早"；P5 已过，时机待重新评估 |

## ✅ P5 收官记录（2026-07-18 上午）

用户回来后明确拍板"现在推"。部署与验收全程：

- **部署**：`git archive` HEAD `a3767a4` → 解压进 `/mnt/nvme0n1-4/docker/subtitle-scout/`（**生产定制版 docker-compose.yml 先备份后还原**——它与 repo 版不同,md5 比对后排除;Dockerfile/.dockerignore 与 repo 一致）→ `docker compose build`（image `39fa4322`）→ 停容器 → **DB 备份 `backup-pre-v8-20260718/`（db+wal+shm,沿 v5/v6 先例）** → `up -d`。
- **迁移**：schema **7→8** 干净落地（item_files 表 + subtitles.file_path 列 + 索引,纯 additive）。
- **验收四口径（部署前基线 → 部署后,全部真机核查）**：
  1. parked duplicate-content 停车行 **40 → 0**（比交接时的 21 又涨到 40,P2 全量自愈转正）;item_files **40 条副本入册**。
  2. SPY×FAMILY（tmdb:120089）：**21 条副本入册**;library 聚合 `coverage.partial:13`;详情 DTO 13 个多文件集带逐文件 `files` 清单（分体态数据面全通）。
  3. 甄别页 duplicate 箱**清零**（triage DTO 无 duplicates 键,仅剩 pending:1/claimed:4 与重复无关）。
  4. **P4b 传播真机生效且安全护栏当场见血**：全库 **5 条副本字幕机械复制成功**（subtitles.file_path 非空行=5）;SPY×FAMILY 主副时长差 10-11s（4K vs 1080p 不同剪辑）被 ±5s 容差**正确拒绝**（宁停不猜,设计行为）;**existsSync 防线在生产上真拦下 2 个用户手放的 Nukitashi sidecar**（e150b73 那道数据损失防线的实战首秀,日志"目标位置已有文件,跳过不覆盖"）。
- **遗留（可选清理,未做）**：`web/src/triage/PendingBox.tsx` duplicates 桶死代码退役（该桶已"空则自动隐藏",纯死代码清理非功能变更）。
- **接手者注意**：部署后日志有 DASHBOARD_TOKEN 未设警告（dashboard 对局域网零鉴权,写端点不受保护）——这是鉴权 A1-A4 时机重估的实证输入。
