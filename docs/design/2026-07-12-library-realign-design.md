...
## 批准记录与追加裁定(2026-07-12)

- 用户批准开工。范围再钉:**主流协议(本地 fs/SMB/NFS)健壮为目标**,长尾协议靠探针+降级+诚实拒绝兜底即可,不追全能。
- **Agent 指导采用 skill 式 playbook**:诊断/整理的 LLM 指导不写成代码里的提示词字符串,而是独立结构化决策手册(`src/agent/playbooks/realign.md`:症状→检查→分类→处方的决策树+范例),agent 模块加载注入;分支逻辑的确定性部分一律在代码闸门,playbook 只管判断槽位。手册可独立评审、版本化、迭代。
- 验收:间谍过家家+NAS 随便折腾;本地 mock 容器需构造**多形态乱排布**测试矩阵(绝对编号平铺/错位/合集文件/特别篇混入/正常库不许误伤)观察 agent 处置。
