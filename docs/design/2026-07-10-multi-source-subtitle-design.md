...
5. **Unified call path: ALL providers go through `subtitle-fetch` CLI, from Phase 1 on** (confirmed 2026-07-10) — the main pipeline never calls provider adapters in-process; it always spawns `subtitle-fetch` (default = fast sources; `--deep` = also anti-bot sources). Rationale: single path, no Phase-2 rewiring of Phase-1 plumbing, agent contract identical in dev and production. Cost: one subprocess spawn (~100ms) per search — negligible against a minutes-scale pipeline. **Consequence for Phase 1 task list:** the `subtitle-fetch` CLI skeleton (arg parsing, adapter registry, JSON stdout contract, fail-soft per provider) moves from Phase 2 into Phase 1; Phase 1 ships it with the assrt/assrt-gems/opensubtitles adapters; Phase 2 only ADDS zimuku/subf2m adapters + the sidecar. Phase 1 task 9 ("pipeline fan-out via in-process Promise.all") is superseded: pipeline.ts spawns the CLI and parses stdout JSON. Provider-level journaling (per-provider api_call records) is emitted by the CLI as structured stderr lines that the pipeline relays into the journal.

6. **Local mock media library = the standard test environment** (user-proposed, confirmed 2026-07-10) — Jellyfin scanning/scraping only needs video-extension filenames parseable to TMDB entries; playable content is irrelevant to the entire acquisition pipeline (mirror → wanted → search → rank → download → sidecar write → refresh/verify). Therefore e2e testing needs NO NAS and NO router: OrbStack runs a local Jellyfin container + scout container + a fixture media volume. **Fixtures are ffmpeg-generated 1-second black clips (~KB each), NOT zero-byte files** — zero-byte files fail ffprobe and leave `MediaStreams` empty, which kills the negative-path tests (already-has-Chinese-embedded / PGS-skip); tiny real clips give real MediaStreams and can even embed a `chi` subtitle track (`-c:s srt -metadata:s:s:0 language=chi`) to exercise skip paths. A fixture-generator script + `docker-compose.local.yml` live in the repo, so any contributor can run the full e2e suite from a bare clone. NAS/router remain production-only surfaces.

---

**Next Step:** Spec approved (all open questions resolved). Invoke `writing-plans` skill to break Phase 1 into concrete implementation tasks (TDD, isolated worktree per implementer, adversarial code review). Phase 2 spec remains design-only until Phase 1 ships.
