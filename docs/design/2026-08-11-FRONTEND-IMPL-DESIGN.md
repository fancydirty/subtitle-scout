...
| triage | 1077 | 5 | 🟡 **雪藏**（代码在、导航里没有、用户看不到） | **保留**，见下方裁决 |
...
#### 裁决 C：`triage/` 1077 行 —— **雪藏保留**（2026-08-13 更正本表原判「删」）

⚠️ **本表此前判「删」，依据是「已被雪藏」——那个依据是错的**，它把整页当成一块死肉。
实测：四个区**分属三族，死活各不相同**，其中 Pending/Excluded 两区的数据面**是活的**
（`parked_paths` 有活写入者：`ingest.ts` 三处 `upsertParkedPath`，经 daemonV2
`requestIngest` 与设置页「立即扫描」两条活路径可达）。同时 `src/v2/subtitleVerifyRepo.ts`
头注释（2026-08-12，更新）把「把 TriagePage 挂回 AppShell」写成 verify 族的**恢复路径**
——本表与那份裁决直接打架。用户裁决：**都留，把冲突文档改一致**。

**处置**：雪藏保留（代码在、导航里没有、用户看不到）。三条保留理由：
它是 verify 族唯一的恢复载体（TimingBox）、dormant 是三页产品里没有后继的显示位、
parked 事实在前端只有这一个读取面。

**🔴 完整裁决（含可证伪的删除判据）是 `web/src/triage/TriagePage.tsx` 的头注释，那里是正本。
本节不重抄——重抄两份必然漂移，本条冲突正是这么来的。** 判据有机器载体：
