...
### 身份纠错：agent 会核验已入库条目的身份

上面说的都是"识别不出"的文件。还有一类更麻烦的：**识别出来了，但识错了**——机械解析在
版权规避乱写（`招z魂z4`）、乱码（`H）后丨室`）、fansub 括号标签、中文标题截断（`铁拳教育`
被截成 `铁.`）这些真实命名上会自信地判错，而错误一旦入库，过去没有任何东西能纠正它。

现在 find-subtitle agent 每次开工前会先核验这批目标的身份：拿文件名、目录名、时长、季集
结构去 TMDB 对账，要求"名字匹配 + 第二条独立证据（季表/年份/时长）吻合"两条都过。核验
不过就自己重新识别，找到正确条目后写一条认领——下一轮扫描按新身份重建条目、清掉错行。

这件事在监控页上是可见的：时间线会出现「corrected the media identity」（绿），「甄别」tab
的「已认领」箱里能看到是 agent 写的还是你手动认领的，每条都能一键撤销。两条安全边界：
agent 的认领**永远不会覆盖你手动认领的**（人的判断是终局），认领范围也不会宽到媒体根
（那样一条认领就钉死整库）。
...
- **NCOP/NCED/PV/CM/Trailer/Menu** 这类命名一律不会被误认成正片——机械层（字幕判定规则 0）始终把它们标成特典并跳过字幕搜索，不会占用救援官的判断预算。
